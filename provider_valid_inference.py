...
class Provider_valid(Dataset):
    ...
    def reset_output(self, default_c=None):
        ...
        if self.model_type == 'superhuman':
            ...
            self.out_embs = np.zeros(tuple([16]+self.raw_data_shape), dtype=np.float32)
            self.out_entropy = np.zeros(tuple([12]+self.raw_data_shape), dtype=np.float32)
    ...
    def add_vol_emb(self, embs_vol):
        fromz, fromy, fromx = self.pos
        if self.num_z < 18:
            raise NotImplementedError


        self.out_embs[:, fromz:fromz+self.out_size[0], \
                            fromx:fromx+self.out_size[1], \
                            fromy:fromy+self.out_size[2]] += embs_vol
        # self.weight_map[:, fromz:fromz+self.out_size[0], \
        #         fromx:fromx+self.out_size[1], \
        #         fromy:fromy+self.out_size[2]] += self.weight_vol
 
    def add_vol_entropy(self, entropy_vol):
        fromz, fromy, fromx = self.pos
        if self.num_z < 18:
            raise NotImplementedError


        self.out_entropy[:, fromz:fromz+self.out_size[0], \
                            fromx:fromx+self.out_size[1], \
                            fromy:fromy+self.out_size[2]] += entropy_vol
        # self.weight_map[:, fromz:fromz+self.out_size[0], \
        #         fromx:fromx+self.out_size[1], \
        #         fromy:fromy+self.out_size[2]] += self.weight_vol
    ...
    def get_results_emb(self):
        # self.out_embs = self.out_embs / self.weight_map
        if self.valid_padding[0] == 0:
            self.out_embs = self.out_embs[:, :, \
                                            self.valid_padding[1]:-self.valid_padding[1], \
                                            self.valid_padding[2]:-self.valid_padding[2]]
        else:
            self.out_embs = self.out_embs[:, self.valid_padding[0]:-self.valid_padding[0], \
                                            self.valid_padding[1]:-self.valid_padding[1], \
                                            self.valid_padding[2]:-self.valid_padding[2]]
        return self.out_embs
