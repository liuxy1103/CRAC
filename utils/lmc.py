...
def mc_baseline(affs, fragments=None,thres=0.36,sigma_seeds=2.0):
    affs = 1 - affs
    ...
    if fragments is None:
        ...
        for z in range(fragments.shape[0]):
            wsz, max_id = ws.distance_transform_watershed(boundary_input[z], threshold=thres, sigma_seeds=sigma_seeds)
            wsz += offset
            offset += max_id
            fragments[z] = wsz
