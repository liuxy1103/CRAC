...
def loss_affinity(pred, pred_T,weightmap,criterion):
    loss0 = criterion(pred[:, 0:1, :, :, :], pred_T[:, 0:1, :, :, :], weightmap[:, 0:1, :, :, :])

    loss1 = criterion(pred[:, 1:2, :, :, :], pred_T[:, 1:2, :, :, :], weightmap[:, 1:2, :, :, :])

    loss2 = criterion(pred[:, 2:3, :, :, :], pred_T[:, 2:3, :, :, :], weightmap[:, 2:3, :, :, :])

    loss = loss0 + loss1 + loss2
    return loss

def embedding_distance_l2(embedding1,embedding2): #B,C,H,W  L2 distance
    dis_emb = torch.norm(embedding1 - embedding2, dim=1)
    loss = torch.mean(dis_emb)
    return loss


def embedding_distance_cos(embedding1,embedding2): #B,C,Z,H,W
    embedding1 = F.normalize(embedding1, p=2, dim=1)
    embedding2 = F.normalize(embedding2, p=2, dim=1)
    loss_map = torch.sum(embedding1 * embedding2, dim=1, keepdim=True)
    loss_map = (loss_map+1)/2
