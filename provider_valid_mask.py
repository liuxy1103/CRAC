...
class Provider_valid(Dataset):
    ...
    def __getitem__(self, index):
        ...
        lb_mask = np.zeros_like(lb, dtype=np.float32)
        lb_mask[lb != 0] = 1.0
        lb_mask = lb_mask[np.newaxis, ...]
        mask_weight = weight_binary_ratio(lb_mask)
        ...
        lb_mask = np.ascontiguousarray(lb_mask, dtype=np.float32)
        mask_weight = np.ascontiguousarray(mask_weight, dtype=np.float32)
        return imgs, lb_affs, weightmap, lb_mask, mask_weight
